...
<template>
  <div class="p-4 sm:p-6 max-w-6xl mx-auto">
    ...
    <div class="flex flex-wrap gap-2 mb-5 bg-white rounded-xl border border-gray-200 p-3">
      ...
      <button class="bg-blue-600 text-white text-sm px-4 py-1.5 rounded-lg hover:bg-blue-700" @click="load">Apply</button>
    </div>
    ...
    <div v-else class="space-y-5">
      ...
      <div v-if="pl" class="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <div class="text-xl font-bold text-red-600">₱{{ fmt(pl.total_expenses || pl.expenses) }}</div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
          <div class="text-xs text-gray-500 mb-1">Net Profit</div>
          <div class="text-xl font-bold" :class="(pl.net_profit || 0) >= 0 ? 'text-green-700' : 'text-red-600'">
            ₱{{ fmt(pl.net_profit) }}
          </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
