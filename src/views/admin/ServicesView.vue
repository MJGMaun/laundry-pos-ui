...
const form = ref({ name: '', category_id: '', pricing_type: 'per_kilo', price: '' })
...
function openForm(svc = null) {
  if (svc) {
    ...
    form.value = { name: '', category_id: '', pricing_type: 'per_kilo', price: '' }
  }
  showForm.value = true
}
    ...
    <Teleport to="body">
      <div v-if="showForm" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
          ...
          <div class="space-y-3">
            <input v-model="form.name" placeholder="Service name *" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
            <select v-model="form.category_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
              <option value="">Select category *</option>
              <option v-for="c in categories" :key="c.id" :value="c.id">{{ c.name }}</option>
            </select>
            <select v-model="form.pricing_type" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
              <option v-for="t in PRICING_TYPES" :key="t.value" :value="t.value">{{ t.label }}</option>
            </select>
            <input v-model="form.price" type="number" step="0.01" min="0" placeholder="Price *" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
          </div>
